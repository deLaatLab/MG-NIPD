...
wd <- paste("/data1/projects/nipd/pipeline/processed_data/family_HBB_", familynr, sep="")
root <- wd

famID <- paste(sep="", "_HBB_", familynr)
fams <- paste(familynr, "_HBB", sep="")
family <- fams
dad <- paste("family", famID, "_D_TLA_HBB", sep="")
mom <- paste("family", famID, "_M_TLA_HBB", sep="")
#cfDNA <- paste("/cfDNA/family", familynr, "_cfDNA_CFTR_Q0.vcf", sep="")
cfDNA <- paste("cfDNA/family", famID, "_cfDNA_HBB.vcf", sep="")

CVSfile <- paste(wd,"/CVS/HBB-CVS",familynr,".all_Q0.vcf", sep="")


dadtla <- paste("/haplotypes/", dad, ".vcf", sep="")
momtla <- paste("/haplotypes/", mom, ".vcf", sep="")

mhap1 <- paste("/haplotypes/HBB-M", familynr, "_lenient_NS7-2.hap1.txt", sep="")
mhap2 <-  paste("/haplotypes/HBB-M", familynr, "_lenient_NS7-2.hap2.txt", sep="")
phap1 <- paste("/haplotypes/HBB-D", familynr, "_lenient_NS7-2.hap1.txt", sep="")
phap2 <- paste("/haplotypes/HBB-D", familynr, "_lenient_NS7-2.hap2.txt", sep="")
...
#extract data for type 2 SNPs (erroneously called type 3 in this section)
...
pdf(file=paste(sep="", fams, "_", "patinh.pdf"))
barplot(height = c((Phap1cnt/Phap1reads)*100, (Phap2cnt/Phap2reads)*100), col=c(rgb(0.6,0,0.6,0.5), rgb(0,0.8,0,0.5)),
        ...
        ylim=c(0,max(c((Phap1cnt/Phap1reads)*100, (Phap2cnt/Phap2reads)*100))+3),
                   ylab= "paternal observations in cfDNA (%)", main=fams)
dev.off()
...
d<- paste ("paternal haplotype 1 fraction:", Phap1cnt , "determined from", Phap1reads, "reads")
e<-paste ("paternal haplotype 2 fraction:", Phap2cnt , "determined from", Phap2reads, "reads")
...
heatmap.2(figurematrix, breaks=c(0, 0.5,1.5, 2.5, 3.5, 4.5), col = c("green", "blue", "orange", "white", "red"),
        Colv=F, Rowv=F,labRow=F, trace="none", ylab=paste("SNPs n=", nrow(figurematrix), sep=""), 
        xlab=paste(sep="",min(figuredata$pos), "-",max(figuredata$pos)), 
